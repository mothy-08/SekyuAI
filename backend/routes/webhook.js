...
router.post("/", async (req, res) => {
  ...
  if (typeof payload?.payload === "string") {
    try {
      ...
      return res
        .status(400)
        .json({ error: "Invalid JSON in urlencoded payload" });
    }
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: `Failed to read watchlist: ${err.message}` });
  }

  if (!watchlist.includes(repoFullName)) {
    return res
      .status(200)
      .json({ message: "Repository not in watchlist, skipping" });
  }
  ...
  if (changedFiles.length === 0) {
    return res
      .status(200)
      .json({ message: "No added or modified files in push" });
  }
  ...
  for (const filePath of changedFiles) {
    ...
    try {
      ...
      results.push({
        file: filePath,
        status: "error",
        message: `Could not fetch file: ${err.message}`,
      });
      continue;
    }
    ...
    try {
      ...
      results.push({
        file: filePath,
        status: "error",
        message: `Failed to create patch PR: ${err.message}`,
      });
    }
  }
